...
import { resetPassword, signInWithPhoneAndPassword } from "@/api/requestFactory/auth";
...
const ResetPassword = () => {
  ...
  async function updatePassword(e) {
    ...
    requestMaker(resetPassword(password, searchParams.get("token")))
      .then((res) => {
        showNotification(
          {
            title: "Success",
        ...
        <section className="w-full md:w-[50%] bg-[#EDF0F8] flex flex-col gap-10 md:gap-28 px-5 md:p-[32px] py-3 md:py-0">
          ...
          <form className="px-2 md:px-[100px]">
            <div className="text-center mb-8">
              <h4 className="font-bold text-[24px] text-[#051438]">Reset Password</h4>
              <p className="text-[16px] text-[#677597] font-medium">Enter a new password of your choice you can remember</p>
            </div>
            ...
            <div className="mb-8">
              <label className="mb-2 text-gray-800 text-sm flex">Confirm Password</label>
              <div className="border bg-white rounded-lg px-4 flex justify-between h-14 hover:border-setly-100 hover:border-2">
                <div className="flex gap-3 items-center w-full">
                  <LockClosedIcon className="h-5 w-5 text-gray-600" />
                  <input name="password" className="w-full flex items-center h-full outline-none text-gray-700"
                    onChange={(e) => setConfirmPassword(e.target.value)} value={confirmPassword} type={confirmPasswordFieldType} />
                </div>
                                      
                              
                <div className="flex items-center pr-2">
                  <button
                    onClick={() => setConfirmPasswordFieldType(confirmPasswordFieldType === 'password' ? 'text' : 'password')}
                    type="button" className="text-gray-400 hover:text-gray-500">
                    <span className="sr-only">
                      Show Password
                    </span>
              ...
              </div>
              {
                ...
                <small className="text-sm text-red-600 mt-1.5">{confirmPasswordError}</small>
              }
            </div>

            <Button text="Update Password" type="submit" onClick={updatePassword} />
          </form>
        </section>
      </main>
